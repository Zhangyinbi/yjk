...
public abstract class BaseActivity extends Activity implements Handler.Callback {
     ...
     * version 0.2
	 * 设置EmptyView
     * 为lisetview 设置 emptyView
     * 这方法在访问listView网络接口 返回数据后才执行的
     * 如下面handleMessage方法
     * @param msg 参数 ，因为访问数据时 不一定只会访问一个网络接口，当某个接口在加载listView数据的接口前加载完成
     * 就会出现 listView还没加载完，就显示了一个emptyView视图，然后再加载出listView
     * 所以用msg.what 判断是否访问的是加载listview数据的接口
     * 例子 com.xiaomizhuang.buildcaptain.activity.MaterialOrderDetailActivity
	 */
    ...
    public boolean handleMessage(Message msg) {
        //emptyView 只有在数据访问完毕 再加载进去，否则会在数据还没加载完 就出现一个类似 没有数据的视图
		if (!isHadEmptyView){
			isHadEmptyView = true;
			setEmptyView(msg);
		}
